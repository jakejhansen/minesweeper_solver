...
class AtariGymEnvironment(BaseEnvironment):
    ...
    def __init__(
        self, random_start=30, display=False, screen_height=84, 
        screen_width=84, game="Breakout-v0", reward_recent_update=100000
    ):

        # Constructor of base class
        super(AtariGymEnvironment, self).__init__() # No arguments to init in
                                                    # BaseEnvironment

        self._display       = display
        self._screen_rgb    = None
        self._reward        = 0.0
        self._done          = False
    ...
    def new_game(self):
        ...
        self._screen_rgb = self.env.reset()
        self.render()
        return self.state
    ...
    def new_random_game(self):
        ...
        self.new_game()
        for _ in range(random.randint(0, self._random_start - 1)):
        #for _ in range(0, self._random_start):
            self._random_step()
        self.render()
        return self.state
    ...
    def _step(self, action):
        ...
        self._screen_rgb, self._reward, self._done, _ = self.env.step(action)
    ...
    def _random_step(self):
        """Takes a random step in the environment and updates all related 
           parameters by calling _step(). Usefull in exploration phase.
    ...
    def act_random(self, is_training=True):
        """Sample a random action from the environment action space.

        Returns
        -------
        int
            Returns the sampled action value.
    ...
    @property
    def lives(self):
        """float: Returns the remaining lives in the game."""
    ...
    @property
    def screen(self):
        """:obj: 'ndarray' of :obj: float: Returns greyscale resized screen."""

        # We use opencv, since it is faster in speed-measurements
        # AREA should better handle downsampling http://tanbakuchi.com/posts/comparison-of-openv-interpolation-algorithms/
        screen_gray = cv2.cvtColor(self._screen_rgb, cv2.COLOR_RGB2GRAY)
        return cv2.resize(screen_gray, (self.screen_height, self.screen_width), interpolation=cv2.INTER_AREA) / 255.0
    ...
    def action_to_string(self, action_id):
        ...
        return self.env.unwrapped.get_action_meanings()[action_id]
