...
learning_rate = 0.001  # you know this by now #0.0005
...
conv1 = tf.layers.conv2d(inputs=input_layer,filters=21,kernel_size=[5, 5],padding="same", activation=tf.nn.relu)
conv2 = tf.layers.conv2d(inputs=conv1,filters=31,kernel_size=[3, 3],padding="same", activation=tf.nn.relu)
conv2_flat = tf.reshape(conv2, [-1, 31*21*3*3])
...
env = Minesweeper(display=False, ROWS = 16, COLS = 16, MINES = 40, OUT = "CONDENSED", rewards = {"win" : 1, "loss" : -1, "progress" : 0.9, "noprogress" : -0.3, "YOLO" : -0.3})
