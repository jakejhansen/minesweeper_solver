...
learning_rate = 0.00001  # you know this by now #0.0005
...
conv2 = tf.layers.conv2d(inputs=conv1,filters=n*n,kernel_size=[4, 4],padding="same", activation=tf.nn.relu)
conv2_flat = tf.reshape(conv2, [-1, 6 * 6 * n*n])
...
env = Minesweeper(display=False, ROWS = 6, COLS = 6, MINES = 7, OUT = "CONDENSED", rewards = {"win" : 10, "loss" : -1, "progress" : 0.9, "noprogress" : -0.3, "YOLO" : -0.3})
